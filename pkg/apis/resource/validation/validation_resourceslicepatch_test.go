	...
	validPatchSpec = resourceapi.ResourceSlicePatchSpec{
		Devices: resourceapi.DevicePatch{
			Filter: &resourceapi.DevicePatchFilter{
				DeviceClass: ptr.To(goodName),
				Driver:      ptr.To("test.example.com"),
				Pool:        ptr.To(goodName),
				Device:      ptr.To(goodName),
			},
			Priority: 100,
			Attributes: map[resourceapi.FullyQualifiedName]resourceapi.DeviceAttribute{
				"test.example.com/int":     {IntValue: ptr.To(int64(42))},
				"test.example.com/string":  {StringValue: ptr.To("hello world")},
				"test.example.com/version": {VersionValue: ptr.To("1.2.3")},
				"test.example.com/bool":    {BoolValue: ptr.To(true)},
			},
...
func TestValidateResourceSlicePatch(t *testing.T) {
	...
	scenarios := map[string]struct {
		...
		"bad-class": {
			wantFailures: field.ErrorList{field.Invalid(field.NewPath("spec", "devices", "filter", "deviceClass"), badName, "a lowercase RFC 1123 subdomain must consist of lower case alphanumeric characters, '-' or '.', and must start and end with an alphanumeric character (e.g. 'example.com', regex used for validation is '[a-z0-9]([-a-z0-9]*[a-z0-9])?(\\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*')")},
			patch: func() *resourceapi.ResourceSlicePatch {
				patch := testResourceSlicePatch(goodName, validPatchSpec)
				patch.Spec.Devices.Filter.DeviceClass = ptr.To(badName)
				return patch
			}(),
		},
		"bad-driver": {
		...
		"bad-attribute-missing-domain": {
			...
			patch: func() *resourceapi.ResourceSlicePatch {
				patch := testResourceSlicePatch(goodName, validPatchSpec)
				patch.Spec.Devices.Attributes = map[resourceapi.FullyQualifiedName]resourceapi.DeviceAttribute{
					resourceapi.FullyQualifiedName(goodName): {StringValue: ptr.To("x")},
				}
				return patch
			}(),
		},
		...
		"bad-attribute-two-values": {
			wantFailures: field.ErrorList{
				field.Invalid(field.NewPath("spec", "devices", "attributes").Key(goodQualifiedName), resourceapi.DeviceAttribute{StringValue: ptr.To("x"), VersionValue: ptr.To("1.2.3")}, "exactly one value must be specified"),
			},
			patch: func() *resourceapi.ResourceSlicePatch {
				patch := testResourceSlicePatch(goodName, validPatchSpec)
				patch.Spec.Devices.Attributes = map[resourceapi.FullyQualifiedName]resourceapi.DeviceAttribute{
					resourceapi.FullyQualifiedName(goodQualifiedName): {StringValue: ptr.To("x"), VersionValue: ptr.To("1.2.3")},
				}
				return patch
			}(),
		},
		"bad-attribute-string-too-long": {
			...
			patch: func() *resourceapi.ResourceSlicePatch {
				patch := testResourceSlicePatch(goodName, validPatchSpec)
				patch.Spec.Devices.Attributes = map[resourceapi.FullyQualifiedName]resourceapi.DeviceAttribute{
					resourceapi.FullyQualifiedName(goodQualifiedName): {StringValue: ptr.To(strings.Repeat("x", resourceapi.DeviceAttributeMaxValueLength+1))},
				}
				return patch
			}(),
		},
		"bad-attribute-version-too-long": {
			wantFailures: field.ErrorList{
				field.Invalid(field.NewPath("spec", "devices", "attributes").Key(goodQualifiedName).Child("version"), strings.Repeat("x", resourceapi.DeviceAttributeMaxValueLength+1), "must be a string compatible with semver.org spec 2.0.0"),
				field.TooLongMaxLength(field.NewPath("spec", "devices", "attributes").Key(goodQualifiedName).Child("version"), strings.Repeat("x", resourceapi.DeviceAttributeMaxValueLength+1), resourceapi.DeviceAttributeMaxValueLength),
			},
			patch: func() *resourceapi.ResourceSlicePatch {
				patch := testResourceSlicePatch(goodName, validPatchSpec)
				patch.Spec.Devices.Attributes = map[resourceapi.FullyQualifiedName]resourceapi.DeviceAttribute{
					resourceapi.FullyQualifiedName(goodQualifiedName): {VersionValue: ptr.To(strings.Repeat("x", resourceapi.DeviceAttributeMaxValueLength+1))},
				}
				return patch
			}(),
		},
		"good-attribute-name-max-length": {
			patch: func() *resourceapi.ResourceSlicePatch {
				patch := testResourceSlicePatch(goodName, validPatchSpec)
				patch.Spec.Devices.Attributes = map[resourceapi.FullyQualifiedName]resourceapi.DeviceAttribute{
					resourceapi.FullyQualifiedName(strings.Repeat("x", resourceapi.DeviceMaxDomainLength) + "/" + strings.Repeat("y", resourceapi.DeviceMaxIDLength)): {StringValue: ptr.To("z")},
				}
				return patch
			}(),
		},
		"bad-attribute-c-identifier": {
			wantFailures: field.ErrorList{
				field.TooLongMaxLength(field.NewPath("spec", "devices", "attributes").Key(goodDomain+"/"+strings.Repeat(".", resourceapi.DeviceMaxIDLength+1)), strings.Repeat(".", resourceapi.DeviceMaxIDLength+1), resourceapi.DeviceMaxIDLength),
				field.TypeInvalid(field.NewPath("spec", "devices", "attributes").Key(goodDomain+"/"+strings.Repeat(".", resourceapi.DeviceMaxIDLength+1)), strings.Repeat(".", resourceapi.DeviceMaxIDLength+1), "a valid C identifier must start with alphabetic character or '_', followed by a string of alphanumeric characters or '_' (e.g. 'my_name',  or 'MY_NAME',  or 'MyName', regex used for validation is '[A-Za-z_][A-Za-z0-9_]*')"),
			},
			patch: func() *resourceapi.ResourceSlicePatch {
				patch := testResourceSlicePatch(goodName, validPatchSpec)
				patch.Spec.Devices.Attributes = map[resourceapi.FullyQualifiedName]resourceapi.DeviceAttribute{
					resourceapi.FullyQualifiedName(goodDomain + "/" + strings.Repeat(".", resourceapi.DeviceMaxIDLength+1)): {StringValue: ptr.To("y")},
				}
				return patch
			}(),
		},
		"bad-attribute-domain": {
			wantFailures: field.ErrorList{
				field.TooLong(field.NewPath("spec", "devices", "attributes").Key(strings.Repeat("_", resourceapi.DeviceMaxDomainLength+1)+"/y"), strings.Repeat("_", resourceapi.DeviceMaxDomainLength+1), resourceapi.DeviceMaxDomainLength),
				field.Invalid(field.NewPath("spec", "devices", "attributes").Key(strings.Repeat("_", resourceapi.DeviceMaxDomainLength+1)+"/y"), strings.Repeat("_", resourceapi.DeviceMaxDomainLength+1), "a lowercase RFC 1123 subdomain must consist of lower case alphanumeric characters, '-' or '.', and must start and end with an alphanumeric character (e.g. 'example.com', regex used for validation is '[a-z0-9]([-a-z0-9]*[a-z0-9])?(\\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*')"),
			},
			patch: func() *resourceapi.ResourceSlicePatch {
				slice := testResourceSlicePatch(goodName, validPatchSpec)
				slice.Spec.Devices.Attributes = map[resourceapi.FullyQualifiedName]resourceapi.DeviceAttribute{
					resourceapi.FullyQualifiedName(strings.Repeat("_", resourceapi.DeviceMaxDomainLength+1) + "/y"): {StringValue: ptr.To("z")},
				}
				return slice
			}(),
		},
		"bad-key-too-long": {
			wantFailures: field.ErrorList{
				field.TooLong(field.NewPath("spec", "devices", "attributes").Key("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx...xxxxxxxxxxxx/yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy"), strings.Repeat("x", resourceapi.DeviceMaxDomainLength+1), resourceapi.DeviceMaxDomainLength),
				field.TooLongMaxLength(field.NewPath("spec", "devices", "attributes").Key("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx...xxxxxxxxxxxx/yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy"), strings.Repeat("y", resourceapi.DeviceMaxIDLength+1), resourceapi.DeviceMaxIDLength),
			},
			patch: func() *resourceapi.ResourceSlicePatch {
				slice := testResourceSlicePatch(goodName, validPatchSpec)
				slice.Spec.Devices.Attributes = map[resourceapi.FullyQualifiedName]resourceapi.DeviceAttribute{
					resourceapi.FullyQualifiedName(strings.Repeat("x", resourceapi.DeviceMaxDomainLength+1) + "/" + strings.Repeat("y", resourceapi.DeviceMaxIDLength+1)): {StringValue: ptr.To("z")},
				}
				return slice
			}(),
		},
		"bad-attribute-empty-domain-and-c-identifier": {
			...
			patch: func() *resourceapi.ResourceSlicePatch {
				patch := testResourceSlicePatch(goodName, validPatchSpec)
				patch.Spec.Devices.Attributes = map[resourceapi.FullyQualifiedName]resourceapi.DeviceAttribute{
					resourceapi.FullyQualifiedName("/"): {StringValue: ptr.To("z")},
				}
				return patch
			}(),
		},
		"combined-attributes-and-capacity-length-max-attributes": {
			patch: func() *resourceapi.ResourceSlicePatch {
				...
				for i := 0; i < resourceapi.ResourceSliceMaxAttributesAndCapacitiesPerDevice; i++ {
					patch.Spec.Devices.Attributes[resourceapi.FullyQualifiedName(fmt.Sprintf("%s/attr_%d", goodDomain, i))] = resourceapi.DeviceAttribute{StringValue: ptr.To("x")}
				}
				return patch
			}(),
		},
		...
		"combined-attributes-and-capacity-length-too-many": {
			wantFailures: field.ErrorList{
				field.Invalid(field.NewPath("spec", "devices"), resourceapi.ResourceSliceMaxAttributesAndCapacitiesPerDevice+1, fmt.Sprintf("the total number of attributes and capacities must not exceed %d", resourceapi.ResourceSliceMaxAttributesAndCapacitiesPerDevice)),
			},
			patch: func() *resourceapi.ResourceSlicePatch {
				patch := testResourceSlicePatch(goodName, validPatchSpec)
				patch.Spec.Devices.Attributes = map[resourceapi.FullyQualifiedName]resourceapi.DeviceAttribute{}
